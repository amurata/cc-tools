> **[English](../../../plugins/dependency-management/commands/deps-audit.md)** | **日本語**

# 依存関係監査とセキュリティ分析

あなたは脆弱性スキャン、ライセンスコンプライアンス、サプライチェーンセキュリティを専門とする依存関係セキュリティエキスパートです。既知の脆弱性、ライセンス問題、古いパッケージについてプロジェクトの依存関係を分析し、実用的な修復戦略を提供します。

## コンテキスト
ユーザーは、プロジェクトの依存関係のセキュリティ脆弱性、ライセンス競合、メンテナンスリスクを特定するための包括的な依存関係分析が必要です。可能な場合は自動修正を伴う実用的な洞察に焦点を当ててください。
...
class LicenseAnalyzer:
    def __init__(self):
        ...
        self.license_restrictions = {
            ...
            'AGPL-3.0': '強力なコピーレフト - ネットワーク使用でソース開示が必要',
            'proprietary': '明示的ライセンスなしで使用不可',
            'unknown': 'ライセンス不明 - 法的レビューが必要'
        }
        ...
        for package_name, package_info in dependencies.items():
            ...
            if license_type in self.license_restrictions:
                issues.append({
                    ...
                    'severity': 'medium',
                    'recommendation': '使用状況を確認してコンプライアンスを確保'
                })
        ...
        try {
            ...
            if (data.size > 1000000) { // 1MB
                sizeAnalysis.recommendations.push({
                    ...
                    suggestion: 'より軽い代替品または遅延読み込みを検討'
                });
            }
        } catch (error) {
...
このPRは、セキュリティ脆弱性と古いパッケージに対処するために{len(updates)}個の依存関係を更新します。
    ...
    - name: Run NPM Audit
      ...
      run: |
        ...
        if [ $(jq '.vulnerabilities.total' npm-audit.json) -gt 0 ]; then
          echo "::error::$(jq '.vulnerabilities.total' npm-audit.json)個の脆弱性が見つかりました"
          exit 1
        fi
        ...
        script: |
          ...
          if (critical > 0) {
            github.rest.issues.create({
              ...
              title: `🚨 ${critical}個のクリティカルな脆弱性が見つかりました`,
              body: '依存関係監査でクリティカルな脆弱性が見つかりました。詳細はワークフロー実行を参照してください。',
