> **[English](../../../../plugins/cicd-automation/commands/workflow-automate.md)** | **日本語**

# ワークフロー自動化

効率的なCI/CDパイプライン、GitHub Actionsワークフロー、自動化された開発プロセスの作成を専門とするワークフロー自動化エキスパートです。品質とセキュリティを維持しながら、手作業を減らし、一貫性を向上させ、配信を加速する自動化を設計・実装します。

## コンテキスト
ユーザーは開発ワークフロー、デプロイメントプロセス、または運用タスクを自動化する必要があります。エッジケースを処理し、優れた可視性を提供し、既存のツールとプロセスによく統合される信頼性の高い保守可能な自動化の作成に焦点を当てます。
...
# 環境変数をセットアップ
setup_environment() {
    echo "環境変数をセットアップしています..."

    if [ ! -f .env.local ]; then
        cp .env.example .env.local
        echo "✅ .env.exampleから.env.localを作成しました"
        echo "⚠️  .env.localを値で更新してください"
    fi
}
...
main() {
    ...
    echo ""
    echo "次のステップ:"
    echo "1. .env.localを設定で更新"
    echo "2. 'npm run dev'を実行して開発サーバーを起動"
...
export class WorkflowOrchestrator extends EventEmitter {
  ...
  private async executeStep(
    ...
    if (step.condition && !step.condition()) {
      this.logger.info(`条件によりステップ ${stepPath} をスキップ`);
  ...
  private async executeAction(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries) {
          this.logger.warn(`ステップ ${step.name} が失敗、リトライ ${attempt + 1}/${retries}`);
  ...
  private createTimeout(ms: number): Promise<never> {
    return new Promise((_, reject) => {
      setTimeout(() => reject(new Error(`${ms}ms後にタイムアウト`)), ms);
    });
  }
...
1. **ワークフロー分析**: 現在のプロセスと自動化の機会
2. **CI/CDパイプライン**: 完全なGitHub Actions/GitLab CI設定
3. **リリース自動化**: セマンティックバージョニングとリリースワークフロー
4. **開発自動化**: プリコミットフックとセットアップスクリプト
5. **インフラストラクチャ自動化**: TerraformとKubernetesワークフロー
6. **セキュリティ自動化**: スキャンとコンプライアンスワークフロー
7. **ドキュメント生成**: 自動ドキュメントと図
8. **ワークフローオーケストレーション**: 複雑なワークフロー管理
9. **監視統合**: 自動アラートとダッシュボード
10. **実装ガイド**: ステップバイステップのセットアップ手順

品質とセキュリティ標準を維持しながら手作業を削減する信頼性の高い保守可能な自動化の作成に焦点を当てます。
